...
author: "Kristoffer Kerkhof"
date: "20 2 2020"
output: pdf_document
---

# Monte Carlo algorithm for the Travelling Salesman problem.
...
```{r}
visibility<- function(matrix,x,y){
  return(1/matrix[x,y])
}

## visibility for the cities
visibility_all <- matrix(nrow = sqrt(length(data2)),ncol = sqrt(length(data2)))

for (i in 1:sqrt(length(data2))) {
  for (j in 1:sqrt(length(data2))) {
    visibility_all[i,j] <- visibility(data2,i,j)
  }
}
#visibility_all
...
ACO <- function(matrix){
  ...
  pheromone_all <- matrix(1,nrow = sqrt(length(data2)),ncol = sqrt(length(data2)))
  
  for (t in 1:100) {
    pheromone_update_all <- matrix(0,nrow = sqrt(length(data2)),ncol = sqrt(length(data2)))
    ...
    for (k in 1:100) {
      ...
      #print(ordering)
      #print(ordering)
      tourplan <- tourplan[ordering,ordering]
      shortest_tour <- tourplan
      L <- distance_overal_matrix(tourplan)
      ...
      for (i in 1:N) {
        if (i!=N) {
          pheromone_update_k[ordering[i],ordering[1+i]]<-1
        }
        pheromone_update_k[ordering[N],ordering[1]]<-1
      }
  ...
  ergebnis <- list(L = L, shortets_tour = shortest_tour,Sold=Sold,iter=iter,best_solution=best_solution,best_path=best_path,matrix=best_path)
  return(ergebnis)
}
```
...
erg$matrix
distance_overal_matrix(erg$best_path)
```
